...
class TelegramWebAppController extends Controller
{
    ...
    public function propertyDetailJson(Request $request, $id)
    {
        ...
        foreach ($property->parameters as $param) {
            ...
            if ((int) $param->id === $areaParamId) {
                $area = $val;
            }
            if ((int) $param->id === $legalParamId) {
                $legal = $val;
            }
            if ((int) $param->id === $directionParamId) {
                $direction = $val;
            }
            if ($val !== '' && $val !== null) {
    ...
    public function adminPropertiesApi(Request $request): JsonResponse
    {
        ...
        $mapped = $properties->map(function (Property $p) {
            ...
            return [
                ...
                'title' => $p->title ?? '',
                'price' => $p->price ?? '',
                'price_raw' => (float) $p->getRawOriginal('price'),
        ...
        if (! empty($referralIds)) {
            ...
            foreach ($commissions as $comm) {
                ...
                if ($comm->property && $comm->property->ward_code) {
                    ...
                    if ($wardName) {
                        $propertyLabel .= ' · '.$this->safeStr($wardName);
                    }
                }
    ...
    private function sendReferralNotification(\App\Models\Customer $referrer, \App\Models\Customer $newUser): void
    {
        try {
            $notifService = app(\App\Services\NotificationService::class);
            $inAppService = app(\App\Services\InAppNotificationService::class);
            ...
            \Log::error('Referral notification failed (authRedirect): '.$e->getMessage());
        }
    }
